...
public class ExtractorJsonLinesTest extends DbTest {
    ...
    public void whenJoinSelectQueryWithRawJsonExecuted_thenJsonLinesResultsReturned() throws Exception {
        try (Connection conn = DriverManager.getConnection(jdbcUrl, username, password);
            ...
            extractor.apply(resultSet, out);
            assertEquals(
                "{\"id\":1,\"name\":\"A\",\"child id\":1,\"data\":{\"a1Data\": "
                    + "{\"name\": \"A1\", \"subject\": {\"name\": {\"lastName\": \"A1S\", \"firstName\": \"Harry\"},"
                    + " \"address\": \"a1Address\"}}},\"child name\":\"A1\"}\n"
                    + "{\"id\":1,\"name\":\"A\",\"child id\":2,\"data\":{\"a2Data\": "
                    + "{\"name\": \"A2\", \"subject\": {\"name\": {\"lastName\": \"A2S\", \"firstName\": \"Harry\"},"
